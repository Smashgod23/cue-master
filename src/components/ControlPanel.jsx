 ...
 *   …all previous props
 */
export default function ControlPanel({ mode, onModeToggle, liveState, connected, micError, onConnect, onDisconnect, onRestart, onOpenNotes, onEndSession, savedCount = 0, userCharName, userCharColor }) {
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        <div className="space-y-3">
          ...
          >
            ...
            Director's Notes
            {savedCount > 0 && (
              <span className="ml-1 inline-flex items-center justify-center min-w-[18px] h-[18px] px-1 rounded-full bg-gold/20 text-gold-deep text-[10px] font-semibold">
                {savedCount}
              </span>
            )}
          </button>

          {/* End Session — opens the saved-notes summary */}
          {onEndSession && (
            <button
              onClick={onEndSession}
              className="w-full flex items-center justify-center gap-2 py-3 px-5 rounded-xl
                bg-ink text-white font-sans font-semibold text-sm
                hover:bg-ink-soft shadow-md shadow-ink/10
                active:scale-[0.98] transition-all duration-200 cursor-pointer"
            >
