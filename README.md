...
After parsing, an autocorrect pass runs pyspellchecker on the dialogue text. It builds a protected-word set from all character names and any word appearing 2+ times (so proper nouns and repeated vocabulary are never altered), then accepts only single-edit-distance corrections. All-caps tokens go through a stricter gate: a correction is only accepted when the single differing character is a known OCR glyph confusion (c/e, i/l, o/0, m/n, etc.), so THC -> THE is repaired but real acronyms like CIA, NASA, and LGBT are left alone. The frontend receives `_corrections` metadata per line so users can see and undo specific changes.
